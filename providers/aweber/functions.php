...
function aweber_object( $id ) {
	...
	if( 'fca_eoi_aweber_get_lists' === K::get_var( 'action', $_REQUEST ) ) {
		$authorization_code = empty( $_REQUEST['aweber_authorization_code'] ) ? '' : sanitize_text_field( $_REQUEST['aweber_authorization_code'] );
		
	} else {
		$fca_eoi = get_post_meta( $id, 'fca_eoi', true );
		$authorization_code = empty( $fca_eoi['aweber_authorization_code'] ) ? '' : trim( K::get_var( 'authorization_code', $api_credentials ) );
	...
	if ( !isSet( $api_credentials['consumerKey'],
...
function aweber_ajax_get_lists() {
	...
	wp_send_json_success( $aweber_object[ 'lists' ] );
}
...
function aweber_integration( $settings ) {
	...
	$authorization_code = empty( $fca_eoi['aweber_authorization_code'] ) ? '' : trim( K::get_var( 'aweber_authorization_code', $fca_eoi ) );
	$list_id = empty( $fca_eoi['aweber_list_id'] ) ? '' : $fca_eoi['aweber_list_id'];
